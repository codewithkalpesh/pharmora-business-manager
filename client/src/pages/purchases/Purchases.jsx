...
export function Purchases() {
  ...
  const { data: stats = { outstandingDues: 0, monthPurchases: 0, dueSoonAmount: 0, dueSoonCount: 0 } } = useQuery({
    ...
    queryFn: () => purchaseApi.getStats({ startDate, endDate }).then((r) => r.data.data),
    staleTime: 60 * 1000,
  });
    ...
    <div className="fade-in space-y-6">
      ...
      <div className="card bg-slate-900/60 border border-slate-800 p-4 flex flex-wrap gap-4 items-center justify-between">
        <div className="flex flex-wrap gap-3 items-center flex-1">
          ...
          {(distributorId || status || startDate || endDate || selectedMonth || search) && (
            <button
              onClick={() => { setDistributorId(''); setStatus(''); setStartDate(''); setEndDate(''); setSelectedMonth(''); setSearch(''); }}
